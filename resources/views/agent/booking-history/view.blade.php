                                            ...
                                            <div class="col-auto pull-right">
                                                <div class="fw-500">
                                                    ...
                                                    <a href="javascript:void(0);" 
                                                       > <i
                                                            class="fa fa-times fa-2x text-success"></i> Cancelled</a>
                                                    @endif  
                                ...
                                <div class=" px-20 py-20 mt-20">
                                    <div class="row y-gap-20 mb-15 mt-15 justify-between">
                                        ...
                                        @if ($order->mail_sent == 1)
                                            <div class="col-auto">
                                                <div class="fw-500">
                                                    <a target="_blank" href="{{ url('storage/app/public/order/' . $order->id . '/vouchers/order-vouchers-' . $order->id . '.pdf') }}" class="button h-50 px-24 -blue-1 bg-blue-1-05 text-blue-1">Download Voucherd</a> 
                                                    <a target="_blank" href="{{ route('agent-invoice-download', $order) }}" class="button h-50 px-24 -blue-1 bg-blue-1-05 text-blue-1 mt-10">Download Invoice</a> 
                                                </div>
                                            </div>
                                        @else
                                        ...
                                        <div class="col-auto">
                                            <div class="fw-500">
                                                <a target="_blank" href="{{ route('agent-invoice-download', $order) }}" class="button h-50 px-24 -blue-1 bg-blue-1-05 text-blue-1">Download Invoice</a> 
                                            </div>
                                        </div>
                                    ...
                                    @foreach ($order->order_rooms_with_cancel as $key => $value)
                                        @php
                                            $child_extra = unserialize($value->child_extra);
                                            $i++;
                                            $child_extra_str = '';
                                            if (is_array($child_extra) && count($child_extra) > 0) {
                                                foreach ($child_extra as $c_key => $c_value) {
                                                    $child_extra_str .= 'Age : ' . $c_value->age . ' CWB : ' . ucfirst($c_value->cwb);
                                                }
                                            }
                                        @endphp
                                        <div class="row y-gap-20 mb-15 mt-15 justify-between border-light rounded-4">
                                            <div class="col-auto">
                                                <div class="fw-500">Room {{ $i }} :

                                                    {{ $value->hotel_details->hotel_name }} :
                                                    <span class="fw-300">{{ $value->room_name }}</span>
                                                    <p>Adult: {{ $value->adult }}</p>
                                                    <p>Child: {{ $value->child . ' - ' . $child_extra_str }}</p>
                                                    <p>From - To :
                                                        {{ date('d/m/Y', strtotime($value->check_in_date)) . ' - ' . date('d/m/Y', strtotime($value->check_out_date)) }}
                                                    </p>
                                                    <p>Nights:
                                                        {{ dateDiffInDays($value->check_in_date, $value->check_out_date) }}
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="col-auto pull-right">
                                    ...
                                    <div class="row mb-15 rounded-8 border-light rounded-4">
                                        <div class="col-12">
                                            <div class="d-flex justify-between ">
                                                <div class="text-15 lh-16">Name</div>
                                                <div class="text-15 lh-16 fw-500 text-blue-1">
